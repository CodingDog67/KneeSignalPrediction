...
from scipy.signal import hanning, hamming, blackman, periodogram, iirdesign, lfilter
...
def plt_vibration_angular_with_segments(vibration, angular, time, fs, seg_ind):
    ...
    for idx in range(0, len(seg_ind)):
        if not idx%2:
            ...
            ax1.axvline(seg_ind[idx]/fs, color='k', linewidth=1)
            ax1.axvspan(seg_ind[idx-1]/fs, seg_ind[idx]/fs, facecolor='#009440', alpha=0.1) # Beautiful LMU color RGB=0.148.64
            # Maxima
...
def plt_segments_in_figure_latex(figure, vibration, angular, time, segments, mycolor, mylinestyle, myhatch):
    (ax1, ax2) = figure.axes
    for segment in segments:
        ax1.axvspan(time[segment[0]], time[segment[1]+1], linewidth=0.25, color=mycolor, fill=False, linestyle=mylinestyle, hatch=myhatch)
        ax2.axvspan(time[segment[0]], time[segment[1]+1], linewidth=0.25, color=mycolor, fill=False, linestyle=mylinestyle, hatch=myhatch)
    return figure
    ...
    with open(csvFilename, 'rU') as f: # open the file in read universal mode
        ...
        for line in f:
            ...
            if FIS_WAVfiletemp[1]=='R':
                ...
                raise Exception('File name in features list does not contain R or L at the expected position. File name: ' + SPLITfilename)
            ...
            if FIS_WAVfile.find("Patella") > -1:
                ...
                raise Exception('File name in features list does not contain expected sensor position File name: ' + SPLITfilename)
...
def calculate_features_in_list_td(filesList, outputFilename, segmentationVersion, loadIgnoreSegmentsNode=True):
    ...
    for item in filesList:
        ...
        (mean, var, std, rms, zcr_0, zcr_std) = my_time_domain_feature_selection(item, segmentationVersion, loadIgnoreSegmentsNode)
        features = item + ","
...
def calculate_features_in_list_fd(filesList, outputFilename, segmentationVersion, loadIgnoreSegmentsNode=True):
    ...
    for item in filesList:
        percent = 100.0 * counter/float(itemsCount)
        print str(percent)+"% - processing: " + item
        (r_10_50, r_25_320, r_40_140, r_50_500, r_300_600, r_500_8k, r_10_100, r_3k_5k, r_6k_8k) = my_frequency_domain_feature_selection(item, segmentationVersion, loadIgnoreSegmentsNode)
        features = item + ","
    ...
    for xmlSegmentationNode in xmlRoot.iter('segmentation'):
        if xmlSegmentationNode is not None:
            if xmlSegmentationNode.attrib['version']==segmentationVersion:
                ...
                break
    if not xmlSegmentationNodeFound:
        print "Could not find Segmentation Node of version: " + segmentationVersion + " for: " + path.basename(signalXmlFileDialogFilename)
        return
    #print xmlSegmentationNodeRequestedVersion
    for segment in xmlSegmentationNodeRequestedVersion.findall('segment'):
         segmentBegin = segment.find('begin').text
...
def vag_ouss_fft_of_signal(signal):
    ...
    window = hanning(numberOfSignalPoints) # windowing the signal helps mitigate spectral leakage (e.g. Hanning, Hamming, Blackman)
